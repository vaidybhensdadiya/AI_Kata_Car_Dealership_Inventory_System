...
import { X, ArrowUpRight, CheckCircle, AlertCircle, ShieldCheck, Car } from 'lucide-react'
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-md">
      <div className="max-w-lg w-full bg-[#151515] border border-white/[0.08] rounded-2xl p-8 relative shadow-2xl overflow-hidden">
        <button
          onClick={onClose}
          className="absolute top-6 right-6 p-2 rounded-xl bg-[#0A0A0A] border border-white/[0.08] text-[#9A9A9A] hover:text-white transition-colors"
        >
          <X className="w-4 h-4" />
        </button>
        ...
        {purchased ? (
          <div className="text-center py-8 space-y-4">
            ...
            <p className="text-[#CBD5E1] text-xs max-w-xs mx-auto">
              Congratulations on purchasing your <strong className="text-[#D98A3D]">{vehicle.make} {vehicle.model}</strong>. AutoVault receipt has been issued.
            </p>
          ...
          <div>
            <div className="flex items-center gap-3 mb-6">
              <div className="w-10 h-10 rounded-xl bg-[#D98A3D]/10 border border-[#D98A3D]/30 flex items-center justify-center text-[#D98A3D]">
                <Car className="w-5 h-5" />
              </div>
              <div>
                <h2 className="font-heading text-xl font-bold text-white">Confirm Purchase</h2>
                <p className="text-xs text-[#9A9A9A]">Review your transaction order details</p>
              </div>
            </div>
            ...
            <div className="bg-[#0A0A0A] rounded-xl p-4 border border-white/[0.08] mb-6 flex items-center justify-between">
              <div>
                <div className="text-[10px] font-bold text-[#D98A3D] uppercase tracking-wider">{vehicle.category}</div>
                <div className="text-base font-bold text-white">{vehicle.make} {vehicle.model}</div>
                <div className="text-xs text-[#9A9A9A] mt-0.5">{vehicle.year || '2024'} Model • In Stock: {vehicle.quantity}</div>
              </div>
              <div className="text-right">
                <div className="text-[10px] text-[#9A9A9A] font-bold uppercase">Ex-Showroom</div>
            ...
            <div className="flex items-center gap-3">
              <button
                onClick={onClose}
                disabled={loading}
                className="w-1/3 py-3 rounded-full bg-[#0A0A0A] border border-white/[0.08] text-[#9A9A9A] font-semibold text-xs hover:text-white transition-colors"
              >
                Cancel
              </button>
              <button
                ...
                disabled={loading}
                className="w-2/3 py-3.5 bg-[#F5F3EF] hover:bg-white text-[#0A0A0A] font-bold text-xs shadow-lg flex items-center justify-center gap-1.5 rounded-full active:scale-[0.98] disabled:opacity-50"
              >
                {loading ? (
